...
// Hidden SVG element used to measure path geometry with the native SVG Path API
// (replaces the previously used Raphael helpers).
let measurePath: SVGPathElement | null = null

const getMeasurePath = (): SVGPathElement => {
  if (!measurePath) {
...
const getTotalLength = (d: string): number => {
  const path = getMeasurePath()
  path.setAttribute('d', d)
  return path.getTotalLength()
}

const getPointAtLength = (d: string, length: number): { x: number; y: number } => {
  const path = getMeasurePath()
  path.setAttribute('d', d)
...
export function clipperPath(fabricObjects: FabricObject[], type: number) {
  // 0: Union 1: Subtract top layer  2: Intersection  3: Exclude overlap
...
const getPathPoints = (item: Path, scaleX: number, scaleY: number, x = 0, y = 0) : ArrayLike<any> => {
  ...
  for (let i = 0; i < itemPath.length; i++) {
    ...
    for (let c = 0; c < totalLength; c += 1) {
      const point = getPointAtLength(d, c)
