<template>
  <div v-if="handleElement" class="space-y-2 border border-border/50 rounded-md p-2.5 bg-muted/20">
    <div class="flex items-center justify-between mb-1">
      ...
      <Badge
        v-if="handleElement.strokeWidth"
        variant="outline"
        class="text-[10px] px-1.5 py-0 font-mono"
      >
        {{ handleElement.strokeWidth }}px
      </Badge>
    </div>
...
const updateStrokeWidth = (strokeWidth: number) => {
  ...
  if (strokeWidth > 0 && !handleElement.value.stroke) {
    handleElement.value.set({ stroke: '#000000', paintFirst: 'stroke' })
  }
  if (strokeWidth === 0) {
    handleElement.value.set({ paintFirst: 'fill' })
  }
  canvas.renderAll()
}
