...
const initThumbCanvas = () => {
  if (thumbCanvas || !thumbnailTemplate.value) return
...
onMounted(() => {
  // If visible is false during mount, the <canvas> isn't in the DOM yet.
  // initThumbCanvas will be called by the visible watcher when it becomes true.
  if (props.visible) {
    initThumbCanvas()
  }
})

// Re-initialize when visible toggles from false → true (progressive loading)
watch(() => props.visible, (newVisible) => {
  if (newVisible) {
    // Canvas element was just inserted — need nextTick for the ref to resolve
    nextTick().then(initThumbCanvas)
  }
})
