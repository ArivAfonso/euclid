...
export class ArcText extends OriginIText {
  ...
  _renderStr(method: any, ctx: CanvasRenderingContext2D, lineIndex: number, charIndex: number, endCharIndex: number, left: number, top: number) {
    const decl = this._getStyleDeclaration(lineIndex, charIndex),
      ...
      isBoth = method === 'both',
      shouldFill = (method === 'fillText' || isBoth) && fullDecl.fill,
      shouldStroke = (method === 'strokeText' || isBoth) && fullDecl.stroke && fullDecl.strokeWidth;
    let fillOffsets, strokeOffsets;
    if (method !== "calc" && !isBoth && !shouldStroke && !shouldFill) {
      return;
    }
    ctx && decl && ctx.save()
