import { hasIndividualCorners, getCornerRadii, applyIndividualCornerClip } from '@/extension/util/cornerClip'
import { ClipPathType } from '@/configs/images'
import { strokeImage } from '@/extension/effects/image.stroke'
import { shadowImage } from '@/extension/effects/image.shadow'
...
export class Image extends OriginImage {
  ...
  private _isBakingEffects = false
  ...
  // Initialization
  initEffects () {
    // Cache original fields once so repeated effect renders are idempotent
    // (re-baking from the pristine source instead of the already-baked one).
  ...
  _render(ctx: CanvasRenderingContext2D) {
    ...
    // Apply corner radius clip for Image objects (native Fabric Image._render ignores rx/ry)
    if (!this.__isCropping) {
      const radii = getCornerRadii(this)
      const hasIndividual = hasIndividualCorners(this)

      if (hasIndividual) {
        ctx.save()
        applyIndividualCornerClip(ctx, width, height, radii)
        super._render(ctx)
        ctx.restore()
      } else {
        const rx = (this as any).rx as number | undefined
        const ry = (this as any).ry as number | undefined
        const hasUniform = (rx !== undefined && rx > 0) || (ry !== undefined && ry > 0)
        if (hasUniform && width > 0 && height > 0) {
    ...
    if (this.effects) {
      ...
      try {
        ...
        for (let i = 0; i < this.effects.length; i++) {
          ...
          if (item.type === 1) {
            await shadowImage(item, this)
          } else {
            await strokeImage(item, this, type)
          }
        }
        this.canvas?.renderAll()
      } finally {
        this._isBakingEffects = false
      }
    }
  }

  // Keep the effects origin in sync when the source is swapped externally
  // (replace image, filters, masks). Internal effect baking is skipped so the
  // pristine source is never overwritten by a baked canvas.
  async setSrc(src: string, options?: { crossOrigin?: string; signal?: AbortSignal }) {
    const prevSrc = this.getSrc()
    await super.setSrc(src, options)
    if (this.effects && !this._isBakingEffects && src !== prevSrc) {
      this.originSrc = src
      this.originWidth = this.width
      this.originHeight = this.height
    }
