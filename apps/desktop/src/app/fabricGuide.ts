...
import { alignLineColor, alignLineWidth } from '@/configs/alignLine'
...
export class FabricGuide extends Disposable {
  ...
  private aligningLineWidth = alignLineWidth
  private aligningLineColor = alignLineColor
  ...
  private objectMoving({ target }: any) {
    ...
    const add = (group: Group | Canvas | StaticCanvas | ActiveSelection) => {
      const objects = group.getObjects().filter((obj) => {
        ...
        // Exclude self and elements in active selection
        ...
        // If element is a group, add its children and exclude the group itself
    ...
    if (check.isActiveSelection(target)) {
      // During an ActiveSelection drag, Fabric temporarily assigns the
      // selection as each object's group. Traversing those groups only sees
      // the selected objects, so there are no external objects to snap to.
      // Traverse the canvas instead; `add` already excludes every selected
  ...
  // When object is rotated, ignore some coordinates. For example, horizontal guides only take top/bottom edge coordinates (ref: figma)
  ...
  /**
   * Check if value1 and value2 are within the specified range, used for alignment line calculation
   */
  ...
  private snap({
    ...
    /** Active object coordinates */
    draggingObjCoords: ACoordsAppendCenter
    /** Horizontal snap point list */
    snapXPoints: Set<number>
    /** Vertical snap point list */
  ...
  private drawLine(x1: number, y1: number, x2: number, y2: number) {
    ...
    // Draw main blue line without white border
